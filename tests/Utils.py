...
def make_test_triple(name: str, size: dict[str, int] = {"x": 10, "y": 10}, period: int = 86400, scale_up: float = .5, scale_down: float = .2, min_replicas: int = 1, max_replicas: int = 10, workload_configs: list[tuple[int, int, str, int]] = [(50, 2000, "mapped", 0)], idle=False) -> list[TestCase]:
    return [
